#+TITLE: RFC-Compliant Email Address Validator for Chibi Scheme
#+SUBTITLE: An email address validator for Chibi Scheme designed for user input validation
...
- *Binary validation*: ~valid-email-address?~ returns ~#true~ (accept)
  or ~#false~ (reject).
- *No FFI required*: Uses Chibi's PEG parsing library.
...
- [[Makefile][Makefile]] - build automation (targets: ~check~, ~clean~, ~help~)
- [[run-tests.scm][run-tests.scm]] - test runner
...
- [[validate.scm][validate.scm]] - validator implementation
- [[validate.sld][validate.sld]] - R7RS library definition
...
- Uses PEG (Parsing Expression Grammar) combinators from ~(chibi parse)~.
